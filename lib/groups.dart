import './element.dart';
import 'package:flutter/material.dart';

final List<TableElement> groupOne = [
  TableElement(
      "H",
      "Hydrogen",
      1,
      1,
      Color(0xFFA3C7D2),
      0.000082,
      1312.05,
      13.99,
      20.27,
      "A colourless, odourless gas.\nIt has the lowest density of all gases"),
  TableElement(
      "Li",
      "Lithium",
      3,
      7,
      Color(0xFFEBBFD8),
      0.534,
      520.22,
      453.65,
      1615.0,
      "A soft, silvery metal.\nIt has the lowest density of all metals.\nIt reacts vigorously with water"),
  TableElement(
      "Na",
      "Sodium",
      11,
      23,
      Color(0xFFEBBFD8),
      0.97,
      495.84,
      370.94,
      1156.09,
      "Sodium is a soft metal that tarnishes within seconds of being exposed to the air.\nIt also reacts vigorously with water"),
  TableElement(
    "K",
    "Potassium",
    19,
    39,
    Color(0xFFEBBFD8),
    0.89,
    418.80,
    336.7,
    1032,
    "A soft, silvery metal that tarnishes in air within minutes",
  ),
  TableElement(
      "Rb",
      "Rubidium",
      37,
      84,
      Color(0xFFEBBFD8),
      1.53,
      403.03,
      312.45,
      961,
      "A soft metal that ignites in the air.\n It reacts violently with water"),
  TableElement(
      "Cs",
      "Caesium",
      55,
      133,
      Color(0xFFEBBFD8),
      1.873,
      375.70,
      301.7,
      944,
      "Caesium is a soft, gold-coloured metal.\nItsis quickly attacked by air and reacts explosively in water"),
  TableElement(
    "Fr",
    "Francium",
    87,
    233,
    Color(0xFFEBBFD8),
    null,
    392.96,
    294,
    923,
    "An intensely radioactive metal",
  ),
];

// TableElement(String symbol, String name, int atomicNum, int atomicMass,
//               Color(0xFFFBC9E5), double density, double ionizationenergy, int melting,
//               int boiling, String appearance)

final List<TableElement> groupTwo = [
  TableElement(
      "Be",
      "Beryllium",
      4,
      9,
      Color(0xFFFBC9E5),
      1.85,
      899.50,
      1560.0,
      2741.0,
      "Beryllium is a silvery-white metal.\nIt is relatively soft and has a low density"),
  TableElement(
      "Mg",
      "Magnesium",
      12,
      24,
      Color(0xFFFBC9E5),
      1.74,
      737.35,
      923.0,
      1363.0,
      "A silvery-white metal that ignites easily\nin air and burns with a bright light"),
  TableElement(
      "Ca",
      "Calcium",
      20,
      40,
      Color(0xFFFBC9E5),
      1.54,
      389.83,
      1115.0,
      1757.0,
      "Calcium is a silvery-white, soft metal.\n It tarnishes rapidly in air and reacts with water"),
  TableElement(
      "Sr",
      "Strontium",
      38,
      88,
      Color(0xFFFBC9E5),
      2.64,
      549.47,
      1050.0,
      1650.0,
      "A soft, silvery metal that burns in air and reacts with water"),
  TableElement(
      "Ba",
      "Barium",
      56,
      137,
      Color(0xFFFBC9E5),
      3.62,
      502.84,
      1000.0,
      2118.0,
      "Barium is a soft, silvery metal.\nIt rapidly tarnishes in air and reacts with water"),
  TableElement(
    "Ra",
    "Radium",
    88,
    236,
    Color(0xFFFBC9E5),
    5.00,
    509.29,
    969.0,
    1773.0,
    "A soft, shiny and silvery radioactive metal",
  ),
];

final List<TableElement> group11 = [
  TableElement(
      "Cu",
      "Copper",
      29,
      63,
      Color(0xFF5BC2E7),
      8.96,
      745.482,
      1357.77,
      2833.0,
      "A reddish-gold metal that is easily worked and drawn into wires."),
  TableElement(
      "Ag",
      "Silver ",
      47,
      107,
      Color(0xFF5BC2E7),
      10.5,
      730.995,
      1234.93,
      2435,
      "Silver is a relatively soft, shiny metal.\nIt tarnishes slowly in air as sulfur compounds react with the surface forming black silver sulfide."),
  TableElement(
      "Au",
      "Gold",
      79,
      196,
      Color(0xFF5BC2E7),
      19.3,
      745.482,
      1337.33,
      3109.0,
      "A soft metal with a characteristic yellow Color. It is chemically unreactive,\nalthough it will dissolve in aqua regia (a mixture of nitric and hydrochloric acids)."),
  TableElement(
    "Rg",
    "Roentgenium",
    111,
    280,
    Color(0xFF5BC2E7),
    0,
    0,
    0,
    0,
    "A highly radioactive metal, of which\nonly a few atoms have ever been made.",
  ),
];
final List<TableElement> group12 = [
  TableElement(
    "Zn",
    "Zinc",
    30,
    65,
    Color(0xFF5BC2E7),
    7.134,
    906.402,
    692.677,
    1180.0,
    "A silvery-white metal with a blue tinge. It tarnishes in air.",
  ),
  TableElement(
      "Cd",
      "Cadmium",
      48,
      112,
      Color(0xFF5BC2E7),
      8.69,
      867.772,
      594.219,
      1040.0,
      "Cadmium is a silvery metal with a bluish tinge to its surface."),
  TableElement(
    "Hg",
    "Mercury",
    80,
    200,
    Color(0xFF5BC2E7),
    13.5336,
    1007.066,
    234.321,
    629.769,
    "A liquid, silvery metal.",
  ),
  TableElement(
    "Cn",
    "Copernicium",
    112,
    285,
    Color(0xFF5BC2E7),
    0,
    0,
    0,
    0,
    "A highly radioactive metal, of which only a few atoms have ever been made.\nIt is thought to be unreactive and more like a noble gas than a metal.",
  ),
];
final List<TableElement> group13 = [
  TableElement(
    "B",
    "Boron",
    5,
    10,
    Color(0xFFCEDC00),
    2.34,
    800.637,
    2350.0,
    4273.0,
    "Pure boron is a dark amorphous powder.",
  ),
  TableElement(
      "Al",
      "Aluminum",
      13,
      26,
      Color(0xFFCEDC00),
      2.7,
      577.539,
      933.473,
      2792.0,
      "Aluminum is a silvery-white, lightweight metal.\nIt is soft and malleable."),
  TableElement(
      "Ga",
      "Gallium",
      31,
      69,
      Color(0xFFCEDC00),
      5.91,
      578.845,
      302.9146,
      2502.0,
      "Gallium is a soft, silvery-white metal,\nsimilar to aluminium."),
  TableElement(
    "In",
    "Indium",
    49,
    114,
    Color(0xFFCEDC00),
    7.31,
    558.299,
    429.75,
    2300.0,
    "A soft, silvery metal that is stable in air and water.",
  ),
  TableElement(
    "Tl",
    "Thallium",
    81,
    204,
    Color(0xFFCEDC00),
    11.8,
    589.351,
    577.0,
    1746.0,
    "A soft, silvery-white metal that tarnishes easily.",
  ),
  TableElement(
    "Nh",
    "Nihonium",
    113,
    286,
    Color(0xFFCEDC00),
    0,
    0,
    0,
    0,
    "A highly radioactive metal, of which only a few atoms have ever been made.",
  ),
];
final List<TableElement> group14 = [
  TableElement(
      "C",
      "Carbon",
      6,
      12,
      Color(0xFFD1A2CB),
      3.513,
      1086.454,
      4098.0,
      4098.0,
      "There are a number of pure forms of this element including graphite,\ndiamond, fullerenes and graphene. "),
  TableElement(
    "Si",
    "Silicon",
    14,
    28,
    Color(0xFFD1A2CB),
    2.3296,
    786.518,
    1687.0,
    3538.0,
    "The element, when ultrapure, is a solid with a blue-grey metallic sheen.",
  ),
  TableElement(
    "Ge",
    "Germanium",
    32,
    72,
    Color(0xFFD1A2CB),
    5.3234,
    762.179,
    1211.4,
    3106.0,
    "A silvery-white semi-metal. It is brittle.",
  ),
  TableElement(
    "Sn",
    "Tin",
    50,
    118,
    Color(0xFFD1A2CB),
    7.287,
    708.581,
    505.078,
    2859.0,
    "A soft, pliable metal. Below 13°C it slowly changes to a powder form.",
  ),
  TableElement(
    "Pb",
    "Lead",
    82,
    207,
    Color(0xFFD1A2CB),
    11.3,
    715.516,
    600.612,
    2022.0,
    "A dull, silvery-grey metal. It is soft and easily worked into sheets.",
  ),
  TableElement(
    "Fl",
    "Flerovium",
    114,
    289,
    Color(0xFFD1A2CB),
    0,
    0,
    0,
    0,
    "A highly radioactive metal, of which only a few atoms have ever been made.",
  ),
];

final List<TableElement> group18 = [
  TableElement(
    "He",
    "Helium",
    2,
    4,
    Color(0xFFEADA24),
    0.000164,
    2372.32,
    0,
    4.22,
    "A colourless, odourless gas that is totally unreactive.",
  ),
  TableElement(
    "Ne",
    "Neon",
    10,
    20,
    Color(0xFFEADA24),
    0.000825,
    2080.66,
    24.56,
    27.104,
    "A colourless, odourless gas. Neon will not react with any other substance",
  ),
  TableElement(
    "Ar",
    "Argon",
    18,
    40,
    Color(0xFFEADA24),
    0.00163,
    1520.57,
    83.81,
    87.302,
    "Argon is a colourless, odourless gas that is totally inert to other substances",
  ),
  TableElement(
    "Kr",
    "Krypton",
    36,
    83,
    Color(0xFFEADA24),
    0.003425,
    1350.75,
    115.78,
    119.74,
    "Krypton is a gas with no colour or smell. It does not react with anything except fluorine gas",
  ),
  TableElement(
    "Xe",
    "Xenon",
    54,
    131,
    Color(0xFFEADA24),
    0.0054,
    1170.35,
    161.4,
    165.05,
    "A colourless, odourless gas. It is very unreactive",
  ),
  TableElement(
    "Rn",
    "Radon",
    86,
    222,
    Color(0xFFEADA24),
    0.0091,
    1037.07,
    202,
    211.5,
    "Radon is a colourless and odourless gas. It is chemically inert, but radioactive",
  ),
  TableElement(
    "Og",
    "Oganesson",
    1181,
    294,
    Color(0xFFEADA24),
    null,
    0,
    0,
    0,
    "A highly radioactive metal, of which only a few atoms have ever been made",
  )
];

final List<TableElement> group17 = [
  TableElement(
    "F",
    " Fluorine",
    9,
    19,
    Color(0xFFEAB37F),
    0.00155,
    1681.04,
    53.48,
    85.04,
    "A very pale yellow-green, dangerously reactive gas. It is the most reactive of all the elements and quickly attacks all metals. Steel wool bursts into flames when exposed to fluorine.",
  ),
  TableElement(
    "Cl",
    "Chlorine",
    17,
    35,
    Color(0xFFEAB37F),
    0.0029,
    1251.18,
    171.7,
    239.11,
    "A yellowy-green dense gas with a choking smell",
  ),
  TableElement(
    "Br",
    "Bromine",
    35,
    80,
    Color(0xFFEAB37F),
    3.102,
    1139.85,
    266,
    332,
    "Bromine is a deep-red, oily liquid with a sharp smell. It is toxic",
  ),
  TableElement(
    "I",
    "Iodine",
    53,
    127,
    Color(0xFFEAB37F),
    4.933,
    1008.39,
    386.9,
    457.6,
    "A black, shiny, crystalline solid. When heated, iodine sublimes to form a purple vapour.",
  ),
  TableElement(
    "At",
    "Astatine",
    85,
    210,
    Color(0xFFEAB37F),
    null,
    0,
    573,
    623,
    "Astatine is a dangerously radioactive element",
  ),
  TableElement(
    "Ts",
    "Tennessine",
    117,
    294,
    Color(0xFFEAB37F),
    null,
    0,
    0,
    0,
    "A highly radioactive metal, of which only a few atoms have ever been made",
  )
];

final List<TableElement> group16 = [
  TableElement(
    "O",
    "Oxygen",
    8,
    16,
    Color(0xFFF0B323),
    0.00131,
    1313.94,
    54.36,
    90.19,
    "A colourless, odourless gas",
  ),
  TableElement(
    "S",
    "Sulfur",
    16,
    32,
    Color(0xFFF0B323),
    2.07,
    999.59,
    388.36,
    717.76,
    "There are several allotropes of sulfur. The most common appears as yellow crystals or powder",
  ),
  TableElement(
    "Se",
    "Selenium",
    34,
    79,
    Color(0xFFF0B323),
    4.81,
    940.96,
    494,
    958,
    "A semi-metal that can exist in two forms: as a silvery metal or as a red powder",
  ),
  TableElement(
    "Te",
    "Tellurium",
    52,
    128,
    Color(0xFFF0B323),
    6.23,
    869.29,
    722.66,
    1261,
    "A semi-metal usually obtained as a grey powder",
  ),
  TableElement(
    "Po",
    "Polonium",
    84,
    209,
    Color(0xFFF0B323),
    9.20,
    811.83,
    527,
    1235,
    "A silvery-grey, radioactive semi-metal",
  ),
  TableElement(
    "Lv",
    "Livermorium",
    116,
    293,
    Color(0xFFF0B323),
    null,
    0,
    0,
    0,
    " A highly radioactive metal, of which only a few atoms have ever been made",
  )
];
